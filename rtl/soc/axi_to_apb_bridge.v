...
module axi_to_apb_bridge (
    ...
    always @(posedge clk) begin
        if (!resetn) begin
            ...
            // Clear responses once CPU acknowledges.  The else branches hold
            // the flag; a later case arm may still override it this cycle,
            // which is the intended priority (last assignment wins).
            if (s_bvalid && s_bready) s_bvalid <= 1'b0;
            else                      s_bvalid <= s_bvalid;
            if (s_rvalid && s_rready) s_rvalid <= 1'b0;
            else                      s_rvalid <= s_rvalid;
            ...
            case (state)
                ...
                IDLE: begin
                    if (aw_acc) begin               // write has priority
                        ...
                        state         <= SETUP;
                    end else begin                  // no request: stay idle
                        apb_psel_r    <= apb_psel_r;
                        apb_penable_r <= apb_penable_r;
                        apb_pwrite_r  <= apb_pwrite_r;
                        apb_paddr_r   <= apb_paddr_r;
                        apb_pwdata_r  <= apb_pwdata_r;
                        apb_pstrb_r   <= apb_pstrb_r;
                        state         <= state;
                    end
                end
                ...
                ACCESS: begin
                    if (PREADY) begin
                        ...
                        // Slave is inserting wait states: hold the APB
                        // signals stable, as required by APB spec 3.1.2.
                        //
                        // Deliberately does NOT re-assign s_bvalid/s_rvalid:
                        // the acknowledge-clear above must win, and in a
                        // sequential block the last assignment is the one
                        // that takes effect.  Holding them here would undo
                        // the clear and wedge the response flags high.
                        apb_psel_r    <= apb_psel_r;
                        apb_penable_r <= apb_penable_r;
                        rdata_r       <= rdata_r;
                        state         <= state;
