...
//   0x0001_0000 – 0x0001_4FFF  App  BRAM 20 KB  (RW, on AXI)
...
//   0x3000_0000 – 0x3000_0FFF  AEAD Wrapper  4 KB  (on APB) TinyJAMBU / Xoodyak / GIFT-COFB / ChaCha20 / Poly1305
//   0x6000_0000 – 0x6000_000C  SD SPI Master     R/W
// ============================================================

module system #(
    parameter BOOT_SIZE = 1024,           // 4 KB in words
    parameter APP_SIZE  = 5120,           // 20 KB in words
    parameter UART_TX_HOLDOFF = 16'd9000,
    // 100 MHz / 868 = 115207 baud (~115200). Use 434 for 50 MHz.
    // Testbenches override with a small value to speed up simulation
    // (baud timing is irrelevant there; the TB snoops uart_we directly).
    parameter integer UART_DEFAULT_DIV = 868
)(
    input            clk_100,
    input            resetn_btn,
    output           trap,
    ...
    // --------------------------------------------------------
    // Clock: 100 MHz board oscillator (clk_100, Arty A7 pin E3)
    // drives the whole system directly through one BUFG.
    // Fallback if 100 MHz does not close timing: restore the
    // clk_div2 /2 divider (50 MHz) + UART DEFAULT_DIV(434) +
    // the create_generated_clock line in synth_system.xdc.
    // --------------------------------------------------------
    wire clk_100g;
    BUFG bufg_in (.I(clk_100), .O(clk_100g));
    ...
    simpleuart #(.DEFAULT_DIV(UART_DEFAULT_DIV)) uart_inst (
        .clk        (clk),
        .resetn     (resetn),
        .ser_tx     (uart_tx),
        .ser_rx     (uart_rx),
    ...
    // --------------------------------------------------------
    // AEAD Wrapper — TinyJAMBU / Xoodyak / GIFT-COFB / ChaCha20 / Poly1305
