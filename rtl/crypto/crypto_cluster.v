...
//   0x014–0x018 : Data_In[127:0]  (Words 13–16)
//   0x01C–0x020 : Data_Out[127:0] (Words 7–10) (Read-Only)
//   0x024       : meas_status     (Word 40) [0]=active, [2:1]=active_alg
//   0x028       : meas_cycles_cur (Word 41) Active clock cycle execution counter
//   0x02C       : meas_cycles_last(Word 42) Cycle count recorded from last run
// ============================================================================
module crypto_cluster #(
    ...
    // Instantiate LiCi Core
    LiCi_Core u_lici_core (
        .iclk     (PCLK),
        .rst_n    (PRESETn),
        .istart   (start_pulse && (alg_sel == 2'b00)), 
        .imode    (decrypt_reg),
        .iData    (reg_data_in),
        .iKey     (reg_key),
        .oData    (core_data_out),
    ...
    // Instantiate Piccolo Core
    Piccolo_Core u_piccolo_core (
        .iclk     (PCLK),
        .rst_n    (PRESETn),
        .istart   (start_pulse && (alg_sel == 2'b01)), 
        .imode    (decrypt_reg),
        .iData    (reg_data_in),
        .iKey     (reg_key),
        .oData    (core_data_out),
    ...
    // Instantiate Klein Core
    Klein_Core u_klein_core (
        .iclk     (PCLK),
        .rst_n    (PRESETn),
        .istart   (start_pulse && (alg_sel == 2'b10)), 
        .imode    (decrypt_reg),
        .iData    (reg_data_in),
        .iKey     (reg_key[63:0]),
        .oData    (core_data_out),
        ...
        if (!PRESETn) begin
            ...
            if (apb_write_strobe) begin
                case (PADDR[9:0])
                    ...
                    // // Data In [64-bit]
                    10'h014: reg_data_in[ 31:  0] <= PWDATA;
                    10'h018: reg_data_in[ 63: 32] <= PWDATA;
    ...
    always @(*) begin
        case (PADDR[9:0])
            ...
            // Read Data Out [64-bit]
            10'h01C: PRDATA = core_data_out[ 31:  0];
            10'h020: PRDATA = core_data_out[ 63: 32];

            // Read Measurement Diagnostics
            10'h024: PRDATA = {29'b0, alg_sel, meas_active};
            10'h028: PRDATA = cycles_cur;
            10'h02C: PRDATA = cycles_last;
